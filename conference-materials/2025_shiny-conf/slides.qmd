...
::: {.notes}
**Code Example**: {bidux} introduction

- Let's look at our Phase 1 implementation of the {bidux} package, which we're releasing today.
- The concept dictionary provides case-insensitive lookups with implementation tips - not just theory, but practical advice on applying each concept.
- You can search across psychology, UI/UX, and data visualization concepts to find what's most relevant to your problem.
- The core of the package is a pipeline of functions that guide you through the 5 BID stages:
  * `bid_notice()` now auto-suggests appropriate theories based on your problem description
  * `bid_interpret()` helps structure your data story with enhanced details
  * `bid_structure()` documents your layout decisions and related concepts
  * Each function provides helpful suggestions as you work
- When we add the upcoming component suggestion system, you'll get tailored UI recommendations based on your BID analysis - showing exactly how to implement these principles in your Shiny code.
- This is just Phase 1 - in Phases 2-4, we'll be adding LLM integration and ready-to-use UI components.
