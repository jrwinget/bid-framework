...
:::: {.columns .nonincremental .fragment}
::: {.column width="50%"}
### Before BID:
<!-- Dashboard with overwhelming filters and complex tables -->
![](img/before-bid.png){width="95%"}
...
::: {.notes}
**Show Impact**: Use before/after contrast to demonstrate value.
...
- Highlight transformative elements in the "after" version:
  * "Notice how there's a clear path through the data"
  * "Progressive disclosure - you see what you need first, then can dig deeper"
  * "The interface itself tells a story about what matters most"
- Mention that this resulted in 43% faster decision-making (preview of results)
:::
...
## Case Study: Stages 1-3 {background-color="#F9F9F9"}

:::: {.columns}
::: {.column width="50%"}
### SoundPulse's Challenges:

- **Stage 1: Notice**
    - Complex filtering overwhelmed users
    - 72% reported feeling confused
- **Stage 2: Interpret**
    - Users couldn't quickly process information
    - Key insights buried in text and numbers
- **Stage 3: Structure**
    - Related elements scattered throughout UI
    - No prioritization of important metrics
:::

::: {.column width="50%"}
### BID Principles Applied:
...
- "Let's see how SoundPulse applied the first three stages of the BID framework"
...
- "Now let's look at how they implemented these principles using {reactable}"
- Transition: "We'll start with a basic implementation and build up to more advanced features"
:::
...
## {reactable} Implementation: Team Collaboration {background-color="#F9F9F9"}

```r
# Comments and annotation feature for Stage 5: Validate & Empower
columns = list(
  # Other columns as before
  comments = colDef(
    name = "Comments",
    cell = function(value, index) {
      comment_id <- paste0("comment-", index)
      tags$div(
        ...
        tags$textarea(
          ...
          value = value,
          onchange = sprintf("saveComment(%s, this.value)", index)
        ),
        tags$div(
          class = "comment-buttons mt-1",
          tags$button(
            type = "button", 
            class = "btn btn-sm btn-outline-primary me-1",
            onclick = sprintf("addHighlight(%s)", index),
            "Highlight"
          ),
          tags$button(
            type = "button",
            class = "btn btn-sm btn-outline-secondary",
            onclick = sprintf("clearComment(%s)", index),
            "Clear"
          )
        )
      )
...
::: {.notes}
**Collaboration Features**: Connect to later BID stages.

- "This example actually implements principles from Stage 5: Validate & Empower through collaboration"
- **WOW MOMENT #4**: "The comment and highlight functionality allows team members to collaborate directly within the dashboard"
- "This addresses the Cooperation & Coordination principle from Stage 5"
- "Notice how we're using custom HTML within reactable cells to create a mini application"
- "The onchange and onclick handlers connect to JavaScript functions for saving comments and highlighting rows"
- "This transforms the dashboard from a static display to a collaborative decision-making tool"
- "Similar features in tools like Google Sheets have transformed how teams work - we can bring that same power to Shiny"
- Transition: "Now that we've seen how to implement Stages 1-3 with reactable, let's move to Stages 4-5..."
:::
...
- **Stage 4: Anticipate**
    - Executives comparing metrics without context
    - Teams interpreting data through confirmation bias
    - Different perspectives needed for different teams
- **Stage 5: Validate**
    - Insights lost after dashboard sessions
    - No clear path to consensus
    - Teams needed actionable summaries
:::
...
::: {.notes}
**Advanced Stages**: Set up context for echarts4r examples.

- "Now we'll look at how SoundPulse tackled the later stages of the BID framework"
- For Stage 4 (Anticipate): "They needed to address how cognitive biases were affecting interpretation"
- For Stage 5 (Validate): "They needed to create lasting impressions and facilitate team consensus"
- "These stages address the subtler psychological aspects of data interpretation"
- "Now let's see how they implemented these principles using {echarts4r}"
- "We'll start with basic chart implementation and build up to more interactive features"
:::
...
::: {.column width="40%"}
<!-- need to either update image or replace with something like the repo README -->
![](img/bid_concept-output.png){width="95%"}
:::
::::
...
- Emphasize this is open source - "We want this to become a community resource"
...
- "This is in active development - we welcome contributors and collaborators"
:::
...
- "In Phase 2, you'll be able to generate UI templates with BID principles built in"
...
- "These aren't just theoretical improvements - SoundPulse measured real results"
- "43% faster decision-making means real business value"
- "67% more insights per session means better ROI on analytics investments"
- "Satisfaction scores jumped from mediocre to excellent"
- "30% faster implementation means developers saved time too"
- When the testimonial appears: "This kind of transformation is what BID is all about"
- "Maya's team went from dreading dashboard development to using it as a competitive advantage"
- "Remember, this wasn't just visual changes - this was a fundamental approach shift"
:::
